(() => {
    ...
    getUpcoming: function(limit = 5) {
      ...
      Object.keys(allData).forEach((dateStr) => {
        ...
        dayTasks.forEach((task) => {
          if (!task.completed) {
            let taskDate = /* @__PURE__ */ new Date(`${dateStr}T00:00:00`);
      ...
      tasksHtml = dateTasks.map((task) => {
        ...
        return `
            <div class="task-item ${task.completed ? "completed" : ""}">
                ...
                <button class="delete-btn" onclick="deleteTaskItem('${selectedDateStr}', '${task.id}')">\xD7</button>
            </div>
            `;
      }).join("");
    }
  ...
  var geminiApiKey = "AIzaSyBqZYednnTzdHX_rcONo5FdJ6OvqszUEss";
  window.sendMessage = async function() {
    ...
    try {
      ...
      const systemPrompt = `You are a helpful calendar assistant. The current date is ${formatDate(/* @__PURE__ */ new Date())}. 
